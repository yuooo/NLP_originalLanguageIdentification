...
By including parse features, this paper reduces the error in NLI up to 30\% compared to using lexical features only.
\subsection*{Data}
The data is the \emph{International Corpus of Learner English} (ICLE), compiled for the task of NLI. All the writers are supposed to have the same English proficiency as well as the same age range. It's a classical corpus in NLI. The dataset they use contains 7 languages, with 95 essays by language. After splitting, they get 17,718 train sentences and 6,791 test sentences.
...
\begin{itemize}
\item a list of function words empirically established in \cite{wong2009contrastive} (binary feature)
\item character bi-grams (best performing n-grams) (frequency feature)
\item the most frequently occurring PoS bi-grams and tri-grams, obtained by using the Brill tagger provided in NLTK being trained on the Brown corpus (binary feature)
\end{itemize}
